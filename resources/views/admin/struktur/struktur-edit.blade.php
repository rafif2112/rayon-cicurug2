        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
                <div class="p-8 sm:p-12">
                    ...
                    <form action="{{ route('struktur.update', $struktur->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        @csrf
                        @method('PUT')
                        <div class="space-y-6">
                            <div>
                                ...
                                <div class="flex justify-center items-center">
                                    <img id="preview" src="{{ asset('assets/images/struktur/' . $struktur->gambar) }}" class="max-h-48 rounded-lg shadow-lg transition-all duration-300 ease-in-out" alt="Pratinjau foto struktur">
                                </div>
                            ...
                            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <input type="text" id="nama" name="nama" value="{{ $struktur->nama }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                    @error('nama')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" id="jabatan" name="jabatan" value="{{ $struktur->jabatan }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                    @error('jabatan')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
