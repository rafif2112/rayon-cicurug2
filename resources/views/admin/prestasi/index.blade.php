<x-admin-layout>
    ...
    <div class="container mx-auto">
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
            @foreach ($prestasi as $item)
                            ...
                            @foreach ($item->gambar as $key => $gambar)
                                @if ($gambar && file_exists(public_path('assets/images/prestasi/' . $gambar)))
                                    <img class="{{ $key === 0 ? 'opacity-100' : 'opacity-0' }} absolute h-full w-full object-cover transition-opacity duration-500 ease-in-out"
                                        data-carousel-item src="{{ asset('assets/images/prestasi/' . $gambar) }}"
                                        alt="{{ $item->nama }}">
                                @else
                                    <img class="{{ $key === 0 ? 'opacity-100' : 'opacity-0' }} absolute h-full w-full object-cover transition-opacity duration-500 ease-in-out"
                            ...
                            <div class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <button type="button"
                                        ...
                                        @click="openModal('{{ route('prestasi.destroy', $item->id) }}', {{ $item->id }})">Delete</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {{-- modal delete --}}
                <div id="modalDelete-{{$item->id}}" class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"
                            ...
                            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                <form id="deleteForm-{{$item->id}}" action="{{ route('prestasi.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                ...
                                <button onclick="closeModal({{ $item->id }})" type="button"
    ...
    <script>
        function openModal(deleteUrl, id) {
            ...
            document.getElementById(`deleteForm-${id}`).action = deleteUrl;
        }
