    ...
    <div class="w-full py-16 text-gray-900">
        <div data-aos="fade-up" class="container mx-auto mb-10 w-11/12 border-b-2 border-black pb-3 sm:w-4/5">
            <div class="mb-3 flex items-center gap-4">
                ...
                <h2 class="text-3xl font-bold text-gray-800 dark:text-white md:text-4xl">Struktur Rayon</h2>
            </div>
        ...
        <div class="container mx-auto flex flex-col items-center justify-center">
            @if ($data->isEmpty())
                <div class="col-span-2 flex flex-col items-center justify-center p-12 text-center">
                    ...
                    <h3 class="text-2xl font-semibold text-gray-400 dark:text-white">Belum ada struktur rayon saat ini</h3>
            ...
            @else
                    ...
                    @foreach ($data as $index => $struktur)
                        @if ($struktur['jabatan'] === 'Pembimbing Siswa')
                            <div class="rounded-lg bg-white p-4 shadow-lg">
                                <div class="relative h-64 w-full mb-4 rounded-t-lg overflow-hidden bg-gray-200">
                ...
                @php
                    $sortedData = collect($data)->sortBy(function ($item) {
                        $order = [
                            'Ketua Rayon' => 1, 'Wakil Ketua Rayon' => 2,
                            'Sekertaris 1' => 3, 'Bendahara 1' => 4,
                            'Sekertaris 2' => 5, 'Bendahara 2' => 6,
                        ];
                ...
                <div class="grid grid-cols-1 gap-x-4 gap-y-6 md:grid-cols-2 md:gap-x-20 lg:grid-cols-3">
                    @foreach ($sortedData as $index => $struktur)
                        @if ($struktur['jabatan'] !== 'Pembimbing Siswa')
                            <div data-aos="fade-up" data-aos-delay="{{ $index * 50 }}" class="rounded-lg bg-white p-4 shadow-lg">
                                <div class="relative h-64 w-full mb-4 rounded-t-lg overflow-hidden bg-gray-200">
                                    @if ($struktur['gambar'] && file_exists(public_path('assets/images/struktur/' . $struktur['gambar'])))
                                        <img src="{{ asset('assets/images/struktur/' . $struktur['gambar']) }}"
                                            alt="{{ $struktur['jabatan'] }}"
